...
async function runRadarAgent(client, options = {}) {
  ...
  const growthTargetFollowers = 1_000_000;
  const growthTargetMultiplier = Math.round((growthTargetFollowers / growthBaselineFollowers) * 100) / 100;
...
async function runStrategistAgent(client, context = {}, options = {}) {
  ...
  const plan = {
    ...
    growthRule: "Meta aspiracional MOONSHOT: atingir 1.000.000 de seguidores em 28 dias. Priorizar conteúdo original de descoberta com motivo forte para seguir. Venda direta é secundária. Se estiver abaixo do ritmo, concentrar testes em Reels originais, séries recorrentes, ganchos de alta curiosidade, compartilhamentos, salvamentos e visitas ao perfil; abandonar rapidamente mecanismos que ficam abaixo da mediana da conta e reaproveitar padrões vencedores sem copiar o criativo. Não comprar seguidores, não usar follow/unfollow, não fazer spam e não aumentar frequência de forma cega.",
