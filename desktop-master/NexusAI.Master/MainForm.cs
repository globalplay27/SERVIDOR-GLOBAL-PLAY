using Microsoft.Web.WebView2.Core;
using Microsoft.Web.WebView2.WinForms;
using System.Diagnostics;

namespace NexusAIMaster;

public sealed class MainForm : Form
{
    private const string NexusMasterUrl = "https://servidor-nexus.diamantehinode2015.workers.dev/master";
    private readonly WebView2 webView = new() { Dock = DockStyle.Fill };

    public MainForm()
    {
        Text = "NEXUS AI MASTER v1.6.0";
        Width = 1420;
        Height = 860;
        MinimumSize = new Size(1040, 700);
        StartPosition = FormStartPosition.CenterScreen;
        BackColor = Color.FromArgb(2, 5, 8);
        Controls.Add(webView);
        Shown += async (_, _) => await StartMasterAsync();
    }

    private async Task StartMasterAsync()
    {
        try
        {
            var dataFolder = Path.Combine(
                Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData),
                "NEXUS AI Master",
                "WebView2"
            );
            Directory.CreateDirectory(dataFolder);

            var environment = await CoreWebView2Environment.CreateAsync(null, dataFolder);
            await webView.EnsureCoreWebView2Async(environment);

            webView.CoreWebView2.Settings.AreDevToolsEnabled = false;
            webView.CoreWebView2.Settings.AreBrowserAcceleratorKeysEnabled = true;
            webView.CoreWebView2.Settings.IsStatusBarEnabled = false;
            webView.CoreWebView2.Settings.IsZoomControlEnabled = true;

            webView.CoreWebView2.NewWindowRequested += (_, e) =>
            {
                e.Handled = true;
                try
                {
                    Process.Start(new ProcessStartInfo(e.Uri) { UseShellExecute = true });
                }
                catch { }
            };

            webView.CoreWebView2.NavigationStarting += (_, e) =>
            {
                if (!Uri.TryCreate(e.Uri, UriKind.Absolute, out var uri)) return;

                if (uri.Host.Equals("servidor-nexus.diamantehinode2015.workers.dev", StringComparison.OrdinalIgnoreCase))
                    return;

                e.Cancel = true;
                try
                {
                    Process.Start(new ProcessStartInfo(e.Uri) { UseShellExecute = true });
                }
                catch { }
            };

            webView.CoreWebView2.NavigationCompleted += (_, e) =>
            {
                if (e.IsSuccess) return;

                MessageBox.Show(
                    "Não foi possível carregar o Painel Master. Verifique sua conexão com a internet e tente novamente.",
                    "NEXUS AI MASTER",
                    MessageBoxButtons.OK,
                    MessageBoxIcon.Error
                );
            };

            webView.CoreWebView2.Navigate(NexusMasterUrl);
        }
        catch (Exception ex)
        {
            MessageBox.Show(
                "Não foi possível iniciar o NEXUS AI MASTER.\n\n" + ex.Message,
                "NEXUS AI MASTER",
                MessageBoxButtons.OK,
                MessageBoxIcon.Error
            );
        }
    }
}
