...
function estimateLunaCost(inputTokens, outputTokens) {
  return (Number(inputTokens || 0) / 1_000_000) * 0.20
    + (Number(outputTokens || 0) / 1_000_000) * 1.20;
}
...
export async function tokenUsageToday(env, clientId) {
  ...
  const configuredLimit = Number(client?.config?.openaiDailyTokenLimit ?? env.NEXUS_OPENAI_DAILY_TOKEN_LIMIT ?? 30000);
  const limitTokens = Number.isFinite(configuredLimit) && configuredLimit > 0 ? Math.floor(configuredLimit) : 30000;
