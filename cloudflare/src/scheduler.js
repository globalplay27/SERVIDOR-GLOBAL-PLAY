...
const LIVE_PUBLISH_TEST_TAG = "live-publish-test-20260925-1722";

async function armOneTimeLivePublishTests(env, now) {
  let armed = 0;
  for (const clientId of ["globalplay-streaming", "ragnar-one"]) {
    const sourceJobId = LIVE_AGENT_TEST_TAG + ":" + clientId;
    const source = await env.DB.prepare(
      "SELECT status FROM scheduled_jobs WHERE id = ?1 LIMIT 1"
    ).bind(sourceJobId).first();
    if (String(source?.status || "") !== "completed") continue;
    ...
    const publishJobId = LIVE_PUBLISH_TEST_TAG + ":" + clientId;
    const result = await env.DB.prepare(
      `INSERT OR IGNORE INTO scheduled_jobs(
         id, client_id, kind, due_at, status, attempts, payload_json, created_at, updated_at
      ...
      clientId,
      now.toISOString(),
      JSON.stringify({ trigger: "manual", testTag: LIVE_PUBLISH_TEST_TAG, postId: candidate.id })
    ).run();
...
export async function runSchedulerTick(env, scheduledAt = new Date()) {
  ...
  const summary = {
    ...
    leadHunterRuns: 0,
    liveTestsQueued: 0,
    livePublishTestsArmed: 0
  };

  summary.liveTestsQueued = await queueOneTimeLiveAgentTests(env, now);
  summary.livePublishTestsArmed = await armOneTimeLivePublishTests(env, now);
